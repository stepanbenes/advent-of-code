pub fn total_distance_between_lists() -> u32 {
    ...
    let result: u32 = left_numbers.iter().zip(right_numbers).map(|(left, right)| left.abs_diff(right)).sum();
...
pub fn similarity_score() -> u32 {
    ...
    let score: u32 = left_numbers.iter().map(|x| x * right_numbers.iter().filter(|&y| x == y).count() as u32).sum();
    score
}
...
fn get_lists() -> (Vec<u32>, Vec<u32>) {
    ...
    let numbers: Vec<Vec<u32>> = lines.map(|line| line.split_whitespace().map(|token| token.parse().unwrap()).collect()).collect();
